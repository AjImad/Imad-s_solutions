...
def calc_tf(t, d):
    """
        t: term frequency in a document
        d: total number of words in document
    """
    return round(t / d, 2)
...
def calc_idf(t, D):
    ...
    """
    return round(math.log(D/(t if t != 0 else 1)), 2)

def calc_tfidf_score(word: str = None) -> float:
    tfs = {} 
    count_word_in_documents = 0
    ...
    for key, doc in documents.items():
        ...
        if word_freq[word]:
            count_word_in_documents += 1

        tfs[key] = calc_tf(word_freq[word], total_doc_words)

    idf = calc_idf(count_word_in_documents, total_docs)

    tf_idfs = {key: round(tfs[key] * idf, 4) for key in documents}
